...
let costs subs positions = tail @@ scanl (fun (count, cost, _) newpos -> (count + subs.?(newpos) `or_default` 0, cost + count, newpos)) (0, 0, 0) positions
...
let solve (initials: list int) =
  ...
  let totalcost = zip_with (fun (countl, costl, posl) (countr, costr, posr) -> costl + costr) c1 c2
  (* c1 *)
