  ...
  for x = dim.x[1], dim.x[2] do
    ...
    for y = dim.y[1], dim.y[2] do
      ...
      for z = dim.z[1], dim.z[2] do
        ...
        res[x][y][z] = count == 3 or (count == 4 and grid[x] and grid[x][y] and grid[x][y][z])
      end
    end
  end
  return res
...
local function countcells(grid, dim)
  ...
  for x = dim.x[1], dim.x[2] do
    for y = dim.y[1], dim.y[2] do
      for z = dim.z[1], dim.z[2] do
        if grid[x] and grid[x][y] and grid[x][y][z] then count = count + 1 end 
      end
    end
  end
  return count
...
do
  ...
  inputpat = grid / function(rawgrid)
    ...
    for i = 1, width do
      res[i+10] = {}
      for j = 1, height do
        res[i+10][j+10] = {[10] = rawgrid[i][j]}
      end
    end
    return {grid = res, dim = {x = {1, width + 20}, y = {1, height + 20}, z = {1, 20}}}
                    end
end
